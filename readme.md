...
* Clearly some sort of heuristics needs to be added as to what the fundamental intent of the suggestion is.  Currently one of the Nerdle Games got the solver into a horrible situation, whereby the answer was: 27+20=47
  ...
  * I should at some stage do some sort of in-depth testing on generated solutions, and how well the Solver copes in order to evaluate changes.  For example, I could just push a "fix" to the solution generator that prevents negative values being allowed in "good" solutions.  This ends up changing the nature of the guesses so much this issue presented here quietly disappears... or more likely, simply shifts onto a different unsolveable solution.
    * Whereas with proper exhaustive testing (proably about the same time I update this antique hardware), I can evaluate how well the Solver copes with "good" puzzles, whether it ever fails, min/max/average attempts before solved and so on.
